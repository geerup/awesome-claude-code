...
framing, Arabic-first (Gulf-familiar MSA, Thmanyah tone), never imply accreditation.
...
│   ├── 01-company-brief.md       Maharat: product, audience, money, owned audience
...
│   ├── brand-voice.md            Arabic-first voice, the no-em-dash house style, visual constants
...
│   ├── copywriter-ar.md          Stream 4: Arabic-first copy (primary)
...
- An instructor layer: the `context/instructors/` registry plus per-instructor packs under
  `skills/instructor-marketing/`, with mining tooling in `commands/` and `scripts/`.
...
- `context/instructors/` (_CATALOG.md registry + per-instructor fact files; mona-ataya mined)
...
- `skills/instructor-marketing/` (hub + mona-ataya pack: SKILL.md, voice.md,
