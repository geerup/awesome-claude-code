# Batch mining protocol: unattended, one subject at a time

The swarm-layer spec for running the full subject mining (mine, QA, review) across
the whole roster in one unattended Claude Code session, without stopping between
subjects. Companion to context/mining-plan-v2.md (the pipeline) and
runtime/SWARM.md (the shapes). Invoked by /mine-all-subjects.
...
- State lives in the repo, not in memory: the catalog pack-status column is the cursor.
  ...
  review passes both pass). A crash mid-subject leaves "mining"; on resume, a
  "mining" row is re-run from its extraction log section (inventory is re-checked, not
  re-trusted).

## Per-subject sequence (three passes, then commit)
...
CONTINUE          next subject. No pause, no prompt.
```
...
- A pass failure returns to the distiller with the exact fixes (verify-then-advance),
  up to 3 repair cycles per subject.
- After 3 failed cycles, or on a blocked folder (nothing readable): write the failure
  ...
  safe (log + queue only, never a half-pack), and continue to the next subject.
...
Mark any subject launched. Produce public-facing assets. Use an unverified claim in
