...
- A live session, a Masterclass launch, or an event needs registrant reminders and a replay
  follow-up (the event-sequence).
- A brief defines a promotion or occasion window (a sale, a seasonal moment, a Skill Path launch,
  a catalog discount) for a multi-instructor offer (the promo-sequence).
...
| Any owned segment, inside a defined promotion or occasion window | Purchases on a multi-instructor, Skill Path, or catalog offer before the window closes | PROMOTION | `promo-sequence` |
...
- `event-sequence`: the event and webinar sequence for registrants and attendees, the
  ...
  limited-time replay. Use for a live session, a Masterclass launch, or an event.
...
- `promo-sequence`: the dated promotion or occasion sequence, a multi-instructor, Skill Path, or
  catalog offer on the escalating-urgency ladder (announce, offer, ends tomorrow, ends tonight,
  optional extension). Each message is a multi-instructor email per
  `context/profiles/maharat/multi-instructor-angles.md`. Use for a sale, a seasonal or occasion campaign, a Skill
  Path launch, or a catalog discount. Selected by a window in the brief, not a lifecycle stage.
...
- The `strategy-artifact` (stream 2): segments of the audience, the angle, offer framing,
  success metric.
- The QA-passed `copy-package` (stream 4): Arabic email copy and subject lines, by variant id.
...
5. Resolve audience size from live data at send time. Record the exact number; the planning
   estimate is about 18,000 non-payers.
6. Surface the platform open item. While unresolved, mark the lifecycle-package design-only
   and not-sendable. Carry the open item forward; never silently close it.
...
audience_size     resolved from owned-audience data (e.g. about 18,000 non-payers)
...
- Gate before advance: skill eval, then `arabic-copy-qa` on all Arabic copy, then
  `brand-qa-reviewer`, then the human gate for the send, per `runtime/verification.md`.

## Hard rules

- The flow sequences copy; it never writes copy. Every message references a QA-passed copy
  variant by id.
- Block on the platform open item before wiring any send. The package is not-sendable until
...
- Never invent the offer, price, schedule, Skill Path title, or instructor name.
