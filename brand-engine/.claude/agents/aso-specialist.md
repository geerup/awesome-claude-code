...
and localization in Arabic and English; the store creative specs (screenshots and app preview,
...
- The QA-passed `copy-package` store strings from copywriter-ar (AR, primary) and copywriter-en
  (EN), and `creative-package` screenshot assets from the designer where the plan needs them.
...
- `qa`: { skill_eval, arabic_qa (store strings via copywriter-ar), english_qa, design_qa (store
  creative), compliance, brand_qa }.
...
- `store`: { `title`, `subtitle`, `description`, `keywords`, `localization` }. Title, subtitle,
  description, and keywords per store, with the English-first localization and the English variant.
  Keywords respect each store's character and field limits.
- `creatives[]`: screenshot and preview specs (dimensions, safe areas, sequence, the message each
  ...
  copywriter-ar. No Arabic text baked into generated images.
...
2. Confirm the app objective, the offer to surface, and the geos and languages from the brief. If
   any is missing, stop and ask. Never assume an offer or a launch claim for the Skill Paths.
...
4. Draft the store strings (title, subtitle, description) by routing Arabic to copywriter-ar and
   English to copywriter-en, then run arabic-copy-qa and brand-qa before they advance.
5. Spec the store creative: screenshot sequence and preview, text-free for generated images, with
   copy-overlay slots for copywriter-ar. Run design-qa.
...
settings.json allowlist change. Arabic capability is the decisive filter for any generative tool.
...
- Failed gate (arabic-copy-qa, design-qa, compliance, or brand-qa): hard stop, return to the author
  (copywriter or designer) with the exact fix list. Fix and resubmit to the same gate.
...
Brief: improve App Store and Google Play visibility in Saudi Arabia, Arabic and English, surfacing
the freemium offer stated in the brief. The plan researches Arabic self-development keywords,
drafts an English-first title and subtitle via copywriter-ar with an English variant via
copywriter-en, and specs a five-frame screenshot sequence where each frame carries one empowering
...
publishes the updated Arabic and English listings to the App Store and Google Play and starts the
subtitle experiment." Nothing publishes until Ahmed approves. The Skill Paths are not announced and
no certificate is implied to be accredited.
...
- Is every generated screenshot frame text-free, with copy in overlay slots from copywriter-ar?
...
- Never invent an offer, price, Skill Path title, or instructor name, and never announce the Skill
  Paths launch. A missing variable is a stop-and-ask.
- Never imply certificate accreditation. No fundraising, roadmap, or unannounced plans in any store
  string, creative, or review reply.
- No Arabic text baked into generated images. Copy lives in overlay slots from copywriter-ar.
