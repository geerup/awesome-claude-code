...
human in build, or set in-build from the QA-passed words by `copywriter-ar` (Arabic) and
`copywriter-en` (English), because generative tools mangle Arabic script and inject tatweel.
...
- The QA-passed copy that fills the slots, when available, from `copywriter-ar` / `copywriter-en`.
...
2. For each concept, write a build-ready spec for Canva or Figma: dimensions and export sizes
   from the asset brief, a grid, a type scale, the color tokens (#141414 background, #1A1A1A
   card surfaces, emerald #009975 accent), and generous space for a premium, uncluttered feel.
3. Position the copy-overlay slots and label each by language (ar / en). Mark the RTL slots so
   Arabic lays out right-to-left. Leave the words to the copywriters; never type Arabic into a
   generated image.
...
- Visual constants applied: #141414, #1A1A1A, emerald #009975, used as accent not flood.
...
scale and spacing, and the fixed color tokens (#141414, #1A1A1A, emerald #009975), with labeled
copy-overlay slots ready for copywriter-ar and copywriter-en. The kit encodes the visual
...
the spec. Generative or build-tool adoption runs through `build-vs-buy-eval` first: Arabic
capability is the decisive filter, adoption is Ahmed's call. The frontmatter `tools:` carries
only the local file tools.
...
- design_spec: Canva, 1080x1350, 12-col grid, #141414 background, one #1A1A1A card, emerald
  #009975 progress mark as the single accent. Type scale set, generous margins.
- copy-overlay slots: one [ar headline] slot positioned top-right, RTL marked; one [en headline]
  slot for the English variant. Words left to the copywriters.
...
- Are the visual constants applied, with emerald as accent, not flood?
...
- No Arabic text inside a generated image. Copy is overlaid by a human or set in-build from
  copywriter-ar / copywriter-en. Generative tools mangle Arabic script and add tatweel.
- Western numerals only in any rendered text. No Eastern Arabic numerals. No tatweel.
- RTL must render correctly. Visual constants #141414, #1A1A1A, emerald #009975.
- No em dashes anywhere in the spec or verdict. Use a comma, a colon, or a period.
- Do not invent Skill Path titles or instructor names. Never imply certificate accreditation.
