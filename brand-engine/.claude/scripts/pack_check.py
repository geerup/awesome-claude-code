#!/usr/bin/env python3
"""Pack checker: deterministic definition-of-done gate for a subject pack.

Validates the full pack for one subject slug per context/mining-plan-v2.md
...
def check(slug):
    ...
    fact = os.path.join(BASE, "context", "subjects", f"{slug}.md")
    pack = os.path.join(BASE, "skills", "subject-marketing", slug)
    ...
    catalog = os.path.join(BASE, "context", "subjects", "_CATALOG.md")
    log = os.path.join(BASE, "references", "subject-extraction-log.md")
    ...
    if os.path.isfile(log) and (slug in open(log, encoding="utf-8").read()
        ...
        ok("extraction log mentions the subject")
    else:
        fails.append("extraction log has no section for this subject")
