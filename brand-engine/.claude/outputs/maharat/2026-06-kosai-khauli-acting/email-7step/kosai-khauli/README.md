# Email 7-step: Kosai Khauli, Teaches Acting (nurture-to-subscribe, slotted build)
...
(`skills/instructor-marketing/kosai-khauli/` SKILL.md, voice.md, masterclass-pages.md) plus
...
- `src`: `https://ic.autopilotapp.com/images/ap-maharat/1_7.png`
- Send-readiness (per `context/instructors/_EMAIL-IMAGE-MANIFEST.md`): BLOCKED: confirm 1_7.png as the header. Both the CloudFront cover and the approved-host autopilot 1_7.png are verified servable; the email uses 1_7.png (approved host). Confirm it as the class header before send.
- Deliverability note: Header is on an approved Ortto host (1_7.png). Confirm 1_7.png as the class header before send.
...
- Every line of copy is a LIVE HTML text slot (`data-slot="copy"`) bound to a stable
  `data-copy-id` (`email-kosai-khauli-<id>-<role>-<lang>`; eyebrow and footer sender are
...
```
python3 .claude/scripts/email_render.py .claude/outputs/2026-06-kosai-khauli-acting/email-7step/kosai-khauli/spec.json
...
- Any series, film, or production title (role/filmography held back, claims row 3 not confirmed)
- Any award or honor
- Any specific lesson count, chapter count, or class duration (page does not enumerate lessons)
- Any price, plan, or subscription term (per-campaign input, not in pack)
- Any date or launch timing
- Any testimonial or named student
- Any promise of roles, fame, or industry access
- Any accreditation of the completion certificate
...
1. Header source: approved Ortto host (ic.autopilotapp.com), verified servable 2026-06-17. Header is on an approved Ortto host (1_7.png). Confirm 1_7.png as the class header before send.
2. Send-readiness (manifest): BLOCKED: confirm 1_7.png as the header. Both the CloudFront cover and the approved-host autopilot 1_7.png are verified servable; the email uses 1_7.png (approved host). Confirm it as the class header before send.
3. Catalog status: kosai-khauli is unconfirmed in context/instructors/_CATALOG.md. Every instructor email is an INTERNAL DRAFT blocked on catalog status, the same status as the existing Bassam build. Nothing sends.
