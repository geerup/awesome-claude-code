# Email 7-step: Toufic Kreidieh, Teaches Building and Growing Your Business (nurture-to-subscribe, slotted build)
...
(`skills/instructor-marketing/toufic-kredieh/` SKILL.md, voice.md, masterclass-pages.md) plus
...
- Header source: approved Ortto host (ic.autopilotapp.com), verified servable 2026-06-17.
- `src`: `https://ic.autopilotapp.com/images/ap-maharat/توفيق كريدية.png`
- Send-readiness (per `context/instructors/_EMAIL-IMAGE-MANIFEST.md`): BLOCKED: confirm the autopilot header. Both the CloudFront cover and an approved-host autopilot image (named pattern) are verified servable; the email uses the autopilot named asset (approved host). Confirm it as the class header before send.
- Deliverability note: Header is on an approved Ortto host (the autopilot named pattern). Confirm it as the class header before send.
...
- Every line of copy is a LIVE HTML text slot (`data-slot="copy"`) bound to a stable
  `data-copy-id` (`email-toufic-kredieh-<id>-<role>-<lang>`; eyebrow and footer sender are
...
```
python3 .claude/scripts/email_render.py .claude/outputs/2026-06-toufic-kredieh-business/email-7step/toufic-kredieh/spec.json
...
- Founder of Brands For Less attribution (voice.md positioning but SKILL.md blocker row 3 held back, homepage-line attribution unconfirmed) kept out of all copy
- Any specific company name, revenue figure, valuation beyond the page-cleared billion-dollar tagline, exit, funding, or award kept out
- Lesson count, chapter count, and class duration kept out (page does not enumerate them, no number invented)
- Price, plan, subscription term, and promotion kept out (none page-cleared, no number invented)
- Any guarantee of business success, revenue, funding, or career outcome kept out (promise limited to skills and confidence)
- Any accreditation of the completion certificate kept out (certificates are not accredited)
- Any testimonial, named student, or specific date kept out (none in pack)
...
1. Header source: approved Ortto host (ic.autopilotapp.com), verified servable 2026-06-17. Header is on an approved Ortto host (the autopilot named pattern). Confirm it as the class header before send.
2. Send-readiness (manifest): BLOCKED: confirm the autopilot header. Both the CloudFront cover and an approved-host autopilot image (named pattern) are verified servable; the email uses the autopilot named asset (approved host). Confirm it as the class header before send.
3. Catalog status: toufic-kredieh is unconfirmed in context/instructors/_CATALOG.md. Every instructor email is an INTERNAL DRAFT blocked on catalog status, the same status as the existing Bassam build. Nothing sends.
