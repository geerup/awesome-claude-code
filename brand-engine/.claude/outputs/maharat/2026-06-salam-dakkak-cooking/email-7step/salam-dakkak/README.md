# Email 7-step: Salam Dakkak, Teaches Levantine Home Cooking (nurture-to-subscribe, slotted build)
...
(`skills/instructor-marketing/salam-dakkak/` SKILL.md, voice.md, masterclass-pages.md) plus
...
- `src` (EN and AR, neutral): `https://dt92b02v6m7lx.cloudfront.net/SD_PLANS_BOTTOMGRADIENT.jpg`
- Send-readiness (per `context/instructors/_EMAIL-IMAGE-MANIFEST.md`): stage to Ortto before send, the same as the other CloudFront covers. SD_PLANS_BOTTOMGRADIENT.jpg is an INTERIM banner (the plans-page image, not a dedicated class cover) until a real Salam class cover is produced. The intended cover dakak-cover.png is DEAD (403, the og:image type).
- Deliverability note: Header is the verified-servable SD_PLANS_BOTTOMGRADIENT.jpg on CloudFront, an interim banner. CloudFront is NOT an approved email host: stage the bytes to the Ortto CDN (or prove with an inbox send-test) and swap the src before any send. The Drive portrait 11032024_SalamBG_141414 is the alternative cover source once hosted. Draft is fine.
...
- Every line of copy is a LIVE HTML text slot (`data-slot="copy"`) bound to a stable
  `data-copy-id` (`email-salam-dakkak-<id>-<role>-<lang>`; eyebrow and footer sender are
...
```
python3 .claude/scripts/email_render.py .claude/outputs/2026-06-salam-dakkak-cooking/email-7step/salam-dakkak/spec.json
...
- No exact recipe lineup or named dishes (page supports only the general 20 plus recipes framing, so no specific dish names were promised)
- No precise lesson, chapter, or recipe count beyond the page-supported 20 plus recipes
- No price, plan tier, or monthly-equivalent figure (none cleared in the pack)
- No subscription start date, launch date, or duration
- No named award body for the Best Female Chef in MENA line; kept exactly as the page phrases it
- No Michelin star claim; kept to the page wording Michelin award winning
- No restaurant-success, income, or career-outcome promise; only recipes, technique, and confidence in the kitchen
- No certificate or accreditation claim
- No testimonial, rating, or student count
- No invented plan-picker URL; step 7 subscribe CTA points to the cleared class page
...
1. Header: the verified-servable `SD_PLANS_BOTTOMGRADIENT.jpg` on CloudFront, an INTERIM banner (the plans-page image, not a dedicated class cover). The intended cover dakak-cover.png is DEAD (403, the og:image type). Stage to the Ortto CDN before send.
2. Send-readiness (manifest): stage to Ortto before send, the same as the other CloudFront covers. SD_PLANS is interim until a real Salam class cover is produced (the Drive portrait 11032024_SalamBG_141414 is the alternative once hosted). Flag for Ahmed.
3. Catalog status: salam-dakkak is unconfirmed in context/instructors/_CATALOG.md. Every instructor email is an INTERNAL DRAFT blocked on catalog status, the same status as the existing Bassam build. Nothing sends.
