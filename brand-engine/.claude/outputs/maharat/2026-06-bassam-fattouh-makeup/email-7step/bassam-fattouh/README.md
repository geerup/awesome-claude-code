# Email 7-step: Bassam Fattouh, Teaches Makeup (nurture-to-subscribe, slotted build)
...
(`skills/instructor-marketing/bassam-fattouh/` SKILL.md, voice.md, masterclass-pages.md) plus
...
- `src`: `https://ic.autopilotapp.com/images/ap-maharat/2_7.png`
- Send-readiness (per `context/instructors/_EMAIL-IMAGE-MANIFEST.md`): yes. Header on the approved Ortto host 2_7.png (verified servable). This is the reference send-ready instructor (manifest). The CLASSCOVER on CloudFront is the design proof; the approved-host 2_7.png is the email src.
- Deliverability note: Header is already on an approved Ortto host (2_7.png). Send-ready per the manifest.
...
- Every line of copy is a LIVE HTML text slot (`data-slot="copy"`) bound to a stable
  `data-copy-id` (`email-bassam-fattouh-<id>-<role>-<lang>`; eyebrow and footer sender are
...
```
python3 .claude/scripts/email_render.py .claude/outputs/2026-06-bassam-fattouh-makeup/email-7step/bassam-fattouh/spec.json
...
- No price, plan, or monthly-equivalent figure stated. The live site and brief do not confirm the plan set for this campaign, so subscribe steps point to the class page as the entry and name no number.
- No lesson count, chapter count, or runtime stated. The page does not enumerate lessons, so the E3 list stays generic to the craft and carries no numbers.
- No award, client, or brand-line attribution. SKILL.md row 3 is blocked and the voice forbids naming private clients or brands, so authority stays anchored to the page tagline only (one of the most sought-after makeup artists in the region).
- No Skill Path title named, no content lineup beyond the page facts, no launch date.
- No accreditation or certificate claim made.
- No free first chapter title asserted as confirmed for this class; E4 frames the free first lesson as the start without binding it to an unconfirmed chapter title.
...
1. Header source: approved Ortto host (ic.autopilotapp.com), verified servable 2026-06-17. Header is already on an approved Ortto host (2_7.png). Send-ready per the manifest.
2. Send-readiness (manifest): yes. Header on the approved Ortto host 2_7.png (verified servable). This is the reference send-ready instructor (manifest). The CLASSCOVER on CloudFront is the design proof; the approved-host 2_7.png is the email src.
3. Catalog status: bassam-fattouh is unconfirmed in context/instructors/_CATALOG.md. Every instructor email is an INTERNAL DRAFT blocked on catalog status, the same status as the existing Bassam build. Nothing sends.
