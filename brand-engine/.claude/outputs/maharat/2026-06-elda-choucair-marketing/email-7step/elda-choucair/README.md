# Email 7-step: Elda Choucair, Teaches Marketing (nurture-to-subscribe, slotted build)
...
(`skills/instructor-marketing/elda-choucair/` SKILL.md, voice.md, masterclass-pages.md) plus
...
- `src`: `https://dt92b02v6m7lx.cloudfront.net/EC_CLASSCOVER_DESKTOP_01_AR-PAGE.webp`
- Send-readiness (per `context/instructors/_EMAIL-IMAGE-MANIFEST.md`): BLOCKED: stage header to Ortto CDN. Both AR and EN headers verified servable on CloudFront only. The existing Elda reference build (the ar2 file) already wires the AR-PAGE cover from CloudFront, so staging that one cover to Ortto unblocks it.
- Deliverability note: Header is the verified CloudFront CLASSCOVER (AR-PAGE for AR, EN-PAGE for EN). CloudFront is NOT an approved email host: stage the bytes to the Ortto CDN (or prove with an inbox send-test) and swap the src before any send. Draft is fine.
...
- Every line of copy is a LIVE HTML text slot (`data-slot="copy"`) bound to a stable
  `data-copy-id` (`email-elda-choucair-<id>-<role>-<lang>`; eyebrow and footer sender are
...
```
python3 .claude/scripts/email_render.py .claude/outputs/2026-06-elda-choucair-marketing/email-7step/elda-choucair/spec.json
...
1. Header source: CloudFront (verified servable at source, NOT an approved email host). Header is the verified CloudFront CLASSCOVER (AR-PAGE for AR, EN-PAGE for EN). CloudFront is NOT an approved email host: stage the bytes to the Ortto CDN (or prove with an inbox send-test) and swap the src before any send. Draft is fine.
2. Send-readiness (manifest): BLOCKED: stage header to Ortto CDN. Both AR and EN headers verified servable on CloudFront only. The existing Elda reference build (the ar2 file) already wires the AR-PAGE cover from CloudFront, so staging that one cover to Ortto unblocks it.
3. Catalog status: elda-choucair is unconfirmed in context/instructors/_CATALOG.md. Every instructor email is an INTERNAL DRAFT blocked on catalog status, the same status as the existing Bassam build. Nothing sends.
