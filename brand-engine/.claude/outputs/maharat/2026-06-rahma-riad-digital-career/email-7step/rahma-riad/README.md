# Email 7-step: Rahma Riad, Teaches Building a Career in the Digital Age (nurture-to-subscribe, slotted build)
...
(`skills/instructor-marketing/rahma-riad/` SKILL.md, voice.md, masterclass-pages.md) plus
...
- `src`: `https://dt92b02v6m7lx.cloudfront.net/RR_CLASSCOVER_DESKTOP_01_LEFTGRADIENT.webp`
- Send-readiness (per `context/instructors/_EMAIL-IMAGE-MANIFEST.md`): BLOCKED: stage header to Ortto CDN, AND catalog status. Header verified servable on CloudFront only. Rahma is also mined-thin and the class existence is unconfirmed in _CATALOG.md; the catalog status check still gates the build.
- Deliverability note: Header is the verified CloudFront CLASSCOVER. CloudFront is NOT an approved email host: stage to the Ortto CDN (or inbox send-test) and swap the src before send. The class existence itself is unconfirmed (mined-thin); confirm in _CATALOG.md. Draft only.
...
- Every line of copy is a LIVE HTML text slot (`data-slot="copy"`) bound to a stable
  `data-copy-id` (`email-rahma-riad-<id>-<role>-<lang>`; eyebrow and footer sender are
...
```
python3 .claude/scripts/email_render.py .claude/outputs/2026-06-rahma-riad-digital-career/email-7step/rahma-riad/spec.json
...
- No lesson list or chapter titles on the page: step 3 ListBlock kept generic to building a career in the digital age, no lesson count or invented chapter names.
- No lesson count, duration, or chapters line on the page: omitted everywhere, no number invented.
- No price, plan, promotion, or date in the pack: step 7 subscribe email carries no price line, no plan name, no deadline; all are stop-and-ask per-campaign inputs.
- No award, figure, named credit, testimonial, or follower count for the reader to gain: none stated. The only follower/empire claim used is the page tagline framed strictly as Rahma's own achievement, never a promise to the reader.
- Free first lesson (E4): the page confirms a Start Watching entry but does not enumerate or title a free intro lesson; E4 points to the Start Watching URL and keeps the free-first-step framing generic, no invented lesson title.
- Certificate not mentioned because the page does not state one for this class; no accreditation implied anywhere.
- voice.md is provisional and singing-oriented; only its verified-safe traits (warm, aspirational, empowering, no fame promise, no talent gatekeeping) were used, nothing contradicting the digital-career class.
- CTA URLs: watch/start steps point to the page-cleared Start Watching URLs (member.maharat.com AR/EN); class-overview and subscribe steps point to the page-cleared class page URLs. No URL invented.
- BLOCKED context: rahma-riad is mined-thin and the class existence itself is unconfirmed in _CATALOG.md; the catalog status check gates this build.
...
1. Header source: CloudFront (verified servable at source, NOT an approved email host). Header is the verified CloudFront CLASSCOVER. CloudFront is NOT an approved email host: stage to the Ortto CDN (or inbox send-test) and swap the src before send. The class existence itself is unconfirmed (mined-thin); confirm in _CATALOG.md. Draft only.
2. Send-readiness (manifest): BLOCKED: stage header to Ortto CDN, AND catalog status. Header verified servable on CloudFront only. Rahma is also mined-thin and the class existence is unconfirmed in _CATALOG.md; the catalog status check still gates the build.
3. Catalog status: rahma-riad is unconfirmed in context/instructors/_CATALOG.md. Every instructor email is an INTERNAL DRAFT blocked on catalog status, the same status as the existing Bassam build. Nothing sends.
