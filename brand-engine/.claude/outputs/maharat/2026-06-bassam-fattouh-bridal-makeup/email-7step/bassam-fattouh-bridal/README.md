# Email 7-step: Bassam Fattouh, Teaches Bridal Makeup (nurture-to-subscribe, slotted build)
...
(`skills/instructor-marketing/bassam-fattouh-bridal/` SKILL.md, voice.md, masterclass-pages.md) plus
...
- `src`: `https://dt92b02v6m7lx.cloudfront.net/BFBRIDAL_CLASSCOVER_DESKTOP_01_LEFTGRADIENT.webp`
- Send-readiness (per `context/instructors/_EMAIL-IMAGE-MANIFEST.md`): BLOCKED: stage header to Ortto CDN. The bridal CLASSCOVER is verified servable on CloudFront only; no approved-host bridal header confirmed yet. Stage it to the Ortto CDN or prove with an inbox send-test before send.
- Deliverability note: Header is the verified CloudFront bridal CLASSCOVER. CloudFront is NOT an approved email host: stage the bytes to the Ortto CDN (or prove with an inbox send-test) and swap the src before any send. Draft is fine.
...
- Every line of copy is a LIVE HTML text slot (`data-slot="copy"`) bound to a stable
  `data-copy-id` (`email-bassam-fattouh-bridal-<id>-<role>-<lang>`; eyebrow and footer sender are
...
```
python3 .claude/scripts/email_render.py .claude/outputs/2026-06-bassam-fattouh-bridal-makeup/email-7step/bassam-fattouh-bridal/spec.json
...
- Lesson count, chapter count, module names, or run time of the bridal class (page does not enumerate them)
- Price, plan name, currency, or any subscription number
- Any promotion, trial, discount, or bundle
- Any start date, deadline, or schedule
- Specific award attribution or named honor for Bassam Fattouh
- Client names, brand-line names, or private-client work
- Testimonials, learner counts, or any social-proof figure
- Certificate or accreditation claims for the bridal class
- Career or professional-employment outcomes; only personal skill and confidence are promised
...
1. Header source: CloudFront (verified servable at source, NOT an approved email host). Header is the verified CloudFront bridal CLASSCOVER. CloudFront is NOT an approved email host: stage the bytes to the Ortto CDN (or prove with an inbox send-test) and swap the src before any send. Draft is fine.
2. Send-readiness (manifest): BLOCKED: stage header to Ortto CDN. The bridal CLASSCOVER is verified servable on CloudFront only; no approved-host bridal header confirmed yet. Stage it to the Ortto CDN or prove with an inbox send-test before send.
3. Catalog status: bassam-fattouh-bridal is unconfirmed in context/instructors/_CATALOG.md. Every instructor email is an INTERNAL DRAFT blocked on catalog status, the same status as the existing Bassam build. Nothing sends.
